...
const StatisticsManager = preload("res://StatisticsManager.gd")
...
# Game Mode Settings
var story_mode_enabled: bool = false  # Story Mode with enhanced narrative
var difficulty_level: int = 1  # 0=Easy, 1=Normal, 2=Hard, 3=Brutal
...
func _ready():
        ...
        # Statistics Manager (era-appropriate dashboards)
        statistics_manager = StatisticsManager.new()
        add_child(statistics_manager)
        statistics_manager.game_manager = self

        # Sound Manager (sound effects)
        sound_manager = SoundManager.new()
        add_child(sound_manager)
        sound_manager.game_manager = self
        sound_manager.connect_to_game_events()

        # Stock Market Manager (aktien/stocks trading)
        stock_market_manager = StockMarketManager.new()
        add_child(stock_market_manager)
        stock_market_manager.game_manager = self

        # AI Competitor Manager (KI-Gegner aus Firmenpool)
        ai_competitor_manager = AICompetitorManager.new()
...
func finish_month():
        ...
        if stock_market_manager: stock_market_manager.process_monthly()

        # Process AI competitors
        if ai_competitor_manager: ai_competitor_manager.process_monthly()
...
func load_game(slot_name: String = "1"):
        ...
        if data.has("player"):
                ...
                difficulty_level = data.player.get("difficulty", 1)
        ...
        if data.has("managers"):
                ...
                if statistics_manager and managers_data.has("statistics"):
                        statistics_manager.load_save_data(managers_data["statistics"])
                if sound_manager and managers_data.has("sound"):
                        sound_manager.load_save_data(managers_data["sound"])
                if stock_market_manager and managers_data.has("stock_market"):
