...
func get_save_data() -> Dictionary:
	return {
		...
		"triggered_events": HISTORICAL_EVENTS.filter(func(e): return e["triggered"]).map(func(e): return {"year": e["year"], "month": e["month"]})
	}

func load_save_data(data: Dictionary):
	newspaper_history = data.get("history", [])
	var triggered = data.get("triggered_events", [])
	for event in HISTORICAL_EVENTS:
		for t in triggered:
			if event["year"] == t["year"] and event["month"] == t["month"]:
				event["triggered"] = true
