...
func _get_yearly_data() -> Dictionary:
	...
	while current_idx < game_manager.history_profit.size():
		var year = start_year + (current_idx / 12)
...
func _draw_line_chart(container: Control, rect: Rect2, data: Array, color: Color, label: String):
	if data.size() < 2:
		return
...
func _draw_text(container: Control, text: String, pos: Vector2, size: int, color: Color):
	container.draw_string(_get_default_font(), pos, text, HORIZONTAL_ALIGNMENT_LEFT, -1, size, color)
