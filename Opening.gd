...
func _show_next_image():
	...
	img_rect.rotation_degrees = rotation
	img_rect.z_index = z_idx
	
	var shadow = _create_shadow(img_rect.position, img_rect.size, rotation)
...
func _create_shadow(pos: Vector2, size: Vector2, rotation: float) -> ColorRect:
