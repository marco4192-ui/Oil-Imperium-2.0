...
# signal dashboard_opened()  # Reserved for future UI integration
...
func _calculate_summary() -> Dictionary:
        var _financial = _calculate_financial_stats()
        var _production = _calculate_production_stats()
...
func _calculate_trend(arr: Array) -> float:
        ...
        var mid_point = arr.size() / 2.0
        var first_half = arr.slice(0, int(mid_point))
        var second_half = arr.slice(int(mid_point))
